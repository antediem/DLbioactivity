...
class TrfmSeq2seq(nn.Module):
    ...
    def _encode(self, src):
        ...
        # mean, max, first*2
        return torch.transpose(output,0,1) # (B,T,H)
#         return torch.hstack((torch.mean(output, dim=0), torch.max(output, dim=0).values, output[0,:,:], penul[0,:,:] )) # (B,4H)
...
#             out = self._encode(src[:,st:ed]) # (B,4H)
            out = self._encode(src[:,st:ed]) # (B,T,H)
            while ed<batch_size:
                st += 100
                ed += 100
                out = torch.cat([out, self._encode(src[:,st:ed])], dim=0)
            return out
